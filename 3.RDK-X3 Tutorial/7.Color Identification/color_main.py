...
change_X = 0   #Change detection area
change_Y = 0  #Change detection area
...
def color_deal():
    ...
    while cap.isOpened():
        ...
        #Draw detection area
        cv2.rectangle(img=frame,pt1=(cx-50,cy-50),pt2=(cx+50,cy+50),color=(0,255,0),thickness=2)
        ...
        #Draw text
        cv2.putText(img=frame,text=color,org=(cx-80,cy-70),fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=1.0,color=(0,255,0),thickness=2)
        #cv2.circle(img=frame,center=(cx,cy),radius=5,color=(0,255,0),thickness=3)

        cv2.imshow('frame',frame)
        key=cv2.waitKey(1) #esc:exit
        if key==27:
            break
    cap.release()
